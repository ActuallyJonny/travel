  ...
  Booking.findOne({ userId: userId }).sort({'createdAt': -1}).exec( (err,booking) => {
    ...
    if (booking) {
      Hotel.findById( booking.hotelId, 'location.city' ).exec( (err, hotel ) => {
        Hotel.find({ 'location.city': hotel.location.city  }, 'name location.city desc rooms.price image reviews.rating').sort('rooms.price').slice('rooms', 1).limit(3).exec( (err, hotels) => {
          res.json(hotels)
        })
      })
    }
